...
watch(searchQuery, (value) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get(route('insumos.index'), { search: value }, {
            preserveState: true,
            replace: true
        });
    }, 300);
...
const confirmDelete = () => {
    if (pendingDeleteId.value !== null) {
        router.delete(route('insumos.destroy', pendingDeleteId.value), {
            preserveScroll: true,
        });
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 w-full space-y-6">
            ...
            <div class="flex items-center gap-2 py-4">
                ...
                <ReportExportButton
                    :data="insumos.data"
                    :headers="['ID', 'Nombre Insumo', 'Proveedor']"
                    :keys="['id', 'nombre', (item) => item.proveedor?.nombre_empresa || 'Sin Proveedor']"
                        ...
                        <tbody class="divide-y divide-sidebar-border">
                            ...
                            <tr v-for="insumo in insumos.data" :key="insumo.id" class="hover:bg-muted/50 transition-colors">
                                ...
                                <td class="p-4 text-right whitespace-nowrap">
                                    <Link :href="route('insumos.edit', insumo.id)" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 mr-3">
                                        <Edit class="h-4 w-4 inline" />
                                    </Link>
                                        <button @click="deleteInsumo(insumo.id)" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
