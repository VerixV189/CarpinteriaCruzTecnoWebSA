...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Permisos', href: route('permisos.index') },
];
...
watch(searchQuery, (value) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get(route('permisos.index'), { search: value }, {
            preserveState: true,
            replace: true
        });
    }, 300);
...
const confirmDelete = () => {
    if (pendingDeleteId.value !== null) {
        router.delete(route('permisos.destroy', pendingDeleteId.value), { preserveScroll: true });
                        ...
                        <tbody class="divide-y divide-sidebar-border">
                            ...
                            <tr v-for="permiso in permisos.data" :key="permiso.id" class="hover:bg-muted/50 transition-colors">
                                ...
                                <td class="p-4 text-right whitespace-nowrap" v-if="currentUserRole === 1">
                                    <Link :href="route('permisos.edit', permiso.id)" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 mr-3">
                                        <Edit class="h-4 w-4 inline" />
                                    </Link>
                                        <button @click="deletePermiso(permiso.id)" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
